    ...
    public class SelectProductFeature : BaseVendingMachineFeature
    {
        public static IEnumerable<object[]> SelectProductData =>
            new List<object[]>
            {
                new object[]
                {
                    new List<Product>() {new Product() {Name = ProductTypes.Cola, Price = 1.00m, AvailableStock = 1}},
                    new List<ICoin>()
                    {
                ...
                new object[]
                {
                    new List<Product>() {new Product() {Name = ProductTypes.Chips, Price = 0.50m, AvailableStock = 1}},
                    new List<ICoin>() {new Coin(USCoinTypes.Quarter), new Coin(USCoinTypes.Quarter)}, ProductTypes.Chips
                },
                new object[]
                {
                    new List<Product>() {new Product() {Name = ProductTypes.Candy, Price = 0.65m, AvailableStock = 1}},
                    new List<ICoin>()
                    {
                        new Coin(USCoinTypes.Quarter), new Coin(USCoinTypes.Quarter), new Coin(USCoinTypes.Nickel),
                        new Coin(USCoinTypes.Dime)
                    },
                    ProductTypes.Candy
                }
            };
        ...
        public void
            GivenTheUserHasInsertedTheCorrectMoney_WhenTheySelectProduct_ThenTheProductIsDispensed_AndThankYouDisplayed(
                IList<Product> products, IList<ICoin> insertedCoins, string productType)
        {
        ...
        [Fact]
        public void
            GivenTheUserHasNotInsertedTheCorrectMoney_WhenTheySelectProduct_ThenTheProductIsNotDispensed_AndPriceDisplayed()
        {
            ...
            _vendingMachine.CurrentDisplay.Should()
                .Be((coins.Sum(s => s.Value)).ToString(CultureInfo.InvariantCulture));
        }

        [Fact]
        public void
            GivenTheUserHasNotInsertedAnyMoney_WhenTheySelectProduct_ThenTheProductIsNotDispensedAndInsertCoinDisplayed()
        {
            var product = new Product() {Name = ProductTypes.Cola, Price = 0.25m, AvailableStock = 1};
            IList<Product> products = new List<Product>() {product};
            _mockUsCurrencyService.Setup(s => s.IsAccepted(It.IsAny<ICoin>())).Returns(true);
